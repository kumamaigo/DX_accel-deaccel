...
class KobukiSlipDetectionTestNode(Node):
  ...
  def __init__(self):
    ...
    self.mu = 0.6  # 路面の静止摩擦係数
    self.h = 0.15  # 重心高さ [m]
    self.Lf = 0.25  # 前軸から重心までの距離 [m]
    self.Lr = 0.45  # 後軸から重心までの距離 [m]
    self.L = self.Lf + self.Lr  # ホイールベース（前軸〜後軸の全幅） [m]

    # --- 加減速の安全係数 ---
    self.alpha = 0.9 # 加速限界にかける安全係数 (1.0で理論限界値)
    self.beta = 0.8   # 減速限界にかける安全係数 (1.0で理論限界値)

    # --- 目標速度プロファイルの設定 ---
    self.v_max1 = 3.6 / 3.6  # 第1段階の目標速度: 3.6 km/h = 1.0 m/s
    self.v_max2 = 15 / 3.6  # 第2段階の目標速度: 10 km/h = 2.78 m/s
