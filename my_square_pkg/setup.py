...
class SquareMoveNode(Node):
    def __init__(self):
        ...
        # KobukiのQoS（RELIABLE, VOLATILE）に合わせたサブスクライバー
        ...
        self.target_distance = 1.5  # 1辺 1.5メートル
        self.target_angle = math.pi / 2.0  # 90度 (rad)

        # 0.1秒周期（10Hz）で制御ループを実行
    ...
    def control_loop(self):
        ...
        if self.state == 'INIT':
            ...
            if dist < self.target_distance:
                twist.linear.x = 0.15  # 前進速度 (0.15 m/s)
            else:
                twist.linear.x = 0.0
            ...
            if angle_diff < self.target_angle:
                twist.angular.z = -0.3  # 右回転（マイナス値, 0.3 rad/s）
            else:
                twist.angular.z = 0.0
                self.cmd_pub.publish(twist)
